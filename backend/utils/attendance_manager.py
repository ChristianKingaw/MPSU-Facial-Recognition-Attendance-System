...
class AttendanceTimeValidator:
    ...
    GRACE_PERIOD_MINUTES = 15  # Minutes after actual start time (when instructor clicked class) to mark as PRESENT
    ABSENT_THRESHOLD_MINUTES = 45  # Minutes after actual start time (when instructor clicked class) to mark as ABSENT
    ...
    def determine_attendance_status(actual_start_time: datetime, checkin_time: datetime) -> str:
        ...
        # If checked in within 15 minutes of actual start time → PRESENT
        if time_diff <= AttendanceTimeValidator.GRACE_PERIOD_MINUTES:
            return "Present"
        elif time_diff >= AttendanceTimeValidator.ABSENT_THRESHOLD_MINUTES:
            # 45 minutes or more after actual start → ABSENT
            return "Absent"
        else:
            # More than 15 minutes but less than 45 minutes after actual start → LATE
            return "Late"
...
class LocationValidator:
    ...
    MAX_DISTANCE_METERS = 50  # Maximum allowed distance from expected location
    ...
    def validate_checkin_location(
        ...
        try:
            # Get class location from database
            ...
            # Get class location coordinates
...
class AttendanceNotifier:
    ...
    def send_notification(
        ...
        try:
            ...
            message = AttendanceNotifier._generate_notification_message(
                instructor, status, checkin_time
            )
            
            # Send through different channels based on notification_type
...
class AttendanceReporter:
    ...
    def generate_attendance_report(
        ...
        try:
            ...
            records = InstructorAttendance.query.filter(
                InstructorAttendance.instructor_id == instructor_id,
                InstructorAttendance.date >= start_date,
                InstructorAttendance.date <= end_date
            ).order_by(InstructorAttendance.date).all()
            ...
            if total_days > 0:
                summary['attendance_rate'] = round(
                    (summary['status_counts']['Present'] / total_days) * 100, 1
                )
                summary['late_rate'] = round(
                    (summary['status_counts']['Late'] / total_days) * 100, 1
                )
            
            return {
                'summary': summary,
                'detailed_records': detailed_records,
                'recommendations': AttendanceReporter._generate_recommendations(summary)
            }
...
class ScheduleManager:
    ...
    def get_scheduled_classes(
        ...
        try:
            ...
            for cls in classes:
                # Parse schedule to check if class is scheduled for this date
            ...
            logging.error(f"Schedule retrieval error: {str(e)}")
            return []
    ...
    def _is_class_scheduled(schedule: str, date: date) -> bool:
        ...
        try:
            ...
            logging.error(f"Schedule parsing error: {str(e)}")
            return False
...
class BackupAuthentication:
    """Handles backup authentication methods when facial recognition fails"""
    
    @staticmethod
    def generate_verification_code(instructor_id: int) -> str:
        """Generate a verification code for manual check-in"""
        import random
        import string
        ...
        verification = VerificationCode(
            instructor_id=instructor_id,
            code=code,
            expires_at=pst_now_naive() + timedelta(minutes=5)
        )
    ...
    @staticmethod
    def verify_code(instructor_id: int, code: str) -> bool:
        ...
        try:
            ...
            if verification.expires_at < pst_now_naive():
                return False
