...
STATUS_WEIGHTS = {
    AttendanceStatus.PRESENT: 0.75,
    AttendanceStatus.LATE: 0.15,
    AttendanceStatus.ABSENT: 0.10,
}
...
def ensure_class_session(class_obj: Class, target_date: date, slot) -> Tuple[Optional[ClassSession], bool]:
    ...
    session = ClassSession(
        ...
        session_room_number=class_obj.room_number,
    )
    db.session.add(session)
    db.session.flush()  # Ensure session.id is available for attendance rows
    return session, True
...
def seed_attendance_for_session(session: ClassSession, enrollments: List[Enrollment]) -> int:
    ...
    for enrollment in enrollments:
        ...
        record = AttendanceRecord(
            student_id=enrollment.student_id,
            class_session_id=session.id,
            date=session.scheduled_start_time,
            status=status,
            time_in=time_in,
            time_out=time_out,
...
def sync_sequence(table_name: str, column_name: str = "id") -> None:
...
def ensure_instructor_attendance(session: ClassSession, class_obj: Class):
    if not class_obj.instructor_id:
        return False
    existing = InstructorAttendance.query.filter_by(
        instructor_id=class_obj.instructor_id,
        class_id=class_obj.id,
        date=session.date,
    ).first()
    ...
    record = InstructorAttendance(
        ...
        status="Present",
        time_in=eleven_am,
        time_out=session.scheduled_end_time,
    )
    db.session.add(record)
    return True
...
def generate_samples(days: int, end_date: date):
    # Ensure sequences are in sync to prevent duplicate PK errors
    sync_sequence("class_sessions")
    sync_sequence("attendance_records")
    sync_sequence("instructor_attendance")
...
def main():
    ...
    with app.app_context():
        ...
        print(f"Created student attendance records: {attendance_created}")
        print(f"Created instructor attendance records: {instructor_created}")
